...
%                               Function
%--------------------------------------------------------------------------
% function [MeanWidthBoth,STDWidthBoth,DI,W,w1,w2,O,o1,o2,H] = OD(file,CF,Window,BrPtWindow);
% INPUT ARGUMENTS:
...
% Eg: >> CF = 20.94
% 3. Window is the window used for line fitting along long-axis
% Eg. >> Window = 5; represents a 5x5 window where a local line is fitted 
% 4. BrPtWindow is the window around the branch point where we avoid measuremtns
% Eg. >> BrPtWindow = 3; represents 3x3 window centered at the branch
% point, where not width measurements are carried out
% OUTPUT:
...
% >> [MeanWidthBoth,STDWidthBoth,DI,W,w1,w2,O,o1,o2] = OD(file,CF,Window,BrPtWindow);
%Example:
...
% It will have subfolders '1' and '2', with output files, data and graphs
...
imwrite(MM,[path,'\' direc '\' 'Eye' num2str(1) '\' 'SelectedROI.bmp']); 
imwrite(MM,[path, '\' direc '\' 'Eye' num2str(2) '\' 'SelectedROI.bmp']);
save([path,'\' direc '\' 'Eye' num2str(1) '\' 'CropXYtest.mat'],'MM','Theta0','angle1');
save([path,'\' direc '\' 'Eye' num2str(1) '\' 'V1V2Angle.mat'],'Theta0');
close all;
cd([path, '\', direc , '\' 'Eye' num2str(1)]);
%Edge-Detection
%-------------------------------------------
...
%              edg = ['edg' num2str(a) '.mat'];
             imwrite(ed(a).f,[path '\' direc '\' 'Eye' num2str(1) '\' 'ODCBordersEye1.bmp']);
             imwrite(ed(a).f,[path, '\' direc '\' 'Eye' num2str(2) '\' 'ODCBordersEye2.bmp']);
             imwrite(edg(a).f,[path '\' direc '\' 'Eye' num2str(1) '\' 'ODCwithEdgeEye1.bmp']);
             imwrite(edg(a).f,[path, '\' direc '\' 'Eye' num2str(2) '\' 'ODCwithEdgeEye2.bmp']);
             save([path,'\' direc '\' 'Eye' num2str(1) '\' 'EdgeSigma.mat'],'sigma');
             break
         end
...
imwrite(ImEdge,[path '\' direc '\' 'Eye' num2str(1) '\' 'FilledODCEye1.bmp']);
imwrite(imcomplement(ImEdge),[path, '\' direc '\' 'Eye' num2str(2) '\' 'FilledODCEye2.bmp']);
close all;
cd([path, '\' direc]);
...
for n2 = 1:2
        ...
        saveas(gcf,[path, '\' direc '\' 'Eye' num2str(n2) '\' 'Binary OCD before (left) and after (right) small region elimination.bmp']);
        close all;
        imshow(bw2);
        imwrite(bw2,[path, '\' direc '\' 'Eye' num2str(n2) '\' 'Binary OCD after small region elimination.bmp']);
        close all;
        cd ..
...
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
        ...
        imwrite(imfuse(bw,skel,'blend'),[path, '\' direc '\' 'Eye' num2str(n2) '\' 'ODCwithSkeletonEye' num2str(n2) '.bmp']);
        close all;
...
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\      
        ...
        saveas(gcf,[path, '\' direc '\' 'Eye' num2str(n2) '\' 'TaggedODColumnsEye' num2str(n2) '.png']);
        close all;
        cd ..
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
...
%                     BB = B{ob,1};%BOUNDARY COORINATE OF OBJECT 'OB'
        ...
        saveas(fig,[path, '\' direc '\' 'Eye' num2str(n2) '\' 'PlinesEye' num2str(n2) '.png']);
        save([path,'\' direc '\' 'Eye' num2str(n2) '\' 'ObWidth.mat'],'ObWidth');
        ...
        saveas(gcf,[path, '\' direc '\' 'Eye' num2str(n2) '\' 'WidthDistriHistogramEye' num2str(n2) '.png']);
        close all;
        ...
        saveas(gcf,[path, '\' direc '\' 'Eye' num2str(n2) '\' 'WidthDistriCDFEye' num2str(n2) '.png']);
        close all;
...
%      VarAngle = 1 - sqrt((sum(sin(z)))^2+(sum(cos(z)))^2)/length(z);
 %     StdAngle = sqrt(-2*log(1-VarAngle))*180/pi;
        ...
        [countsAng,centersAng] = hist(z*180/pi,nbins);
        ...
        saveas(gcf,[path, '\' direc '\' 'Eye' num2str(n2) '\' 'AngleDistiHistogramEye' num2str(n2) '.png']);%xlim([-ThetaV12(n1,2)*180./pi 180+ThetaV12(n1,2)*180/pi]);
        save([path,'\' direc '\' 'Eye' num2str(n2) '\' 'GlobWidth.mat'],'GlobWidth','MeanODW','StdODW','wid','counts','centers','Theta','countsAng','centersAng','MeanAngle','entropy','G'); 
        close all;
        
end
mkdir([path, '\' direc '\' 'BothEyes']);
H = zeros(1,7);
W = [w1 ; w1];
O = [o1 ; o2];
...
[countsAngBoth,centersAngBoth] = hist(O*180/pi,nbins);
...
saveas(gcf,[path, '\' direc '\' 'BothEyes' '\' 'WidthDistriHistogramBothEyes.png']);
close all;
...
saveas(gcf,[path, '\' direc '\' 'BothEyes' '\' 'AngleDistriHistogramBothEyes.png']); close all;
cd(path);
save([path,'\' direc '\' 'BothEyes' '\' 'CombinedWidth.mat'],'O','W','entropyBoth','H','DI'); 
if xl == 1
        cd([path,'\' direc '\' 'BothEyes']);
        ...
        OutputAngle  = [HeaderAngle; num2cell(W*180/pi)];
