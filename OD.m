...
% 8. borderAngle (the function calculating the orientation of ODC pattern) 
%- -------------------------------------------------------------------------
%                               Function
%--------------------------------------------------------------------------
% function [MeanWidthBoth,STDWidthBoth,DI,W,w1,w2,O,H] = OD(file,CF);
% INPUT ARGUMENTS:
...
% 8.H - a 1x6 array with some above information. Elements are: 
...
% >> [MeanWidthBoth,STDWidthBoth,DI,W,w1,w2,O] = OD(file,CF);
%Example:
% >> [MeanWidthBoth,STDWidthBoth,DI,W,w1,w2,O] = OD('C:\Users\Admin\Desktop\A\OD.png',20.95,5,3);
...
function [MeanWidthBoth,STDWidthBoth,CI,W,w1,w2,O,H] = OD(file,CF);
close all; clc;
...
for n2 = 1:2
        ...
        close all;
        odBorder = imread(['ODCBordersEye' num2str(n2) '.bmp']);
        cd ..
...
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\      
        ...
        cd ..
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
%BORDER ANGLE CALCULATIONS FOR COMPLEXITY COMPUTATION
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\   
        if n2 == 1
        [borderAng] = borderAngle(bw, odBorder, wsize, Theta0);%calculates the orientation of OD borders
        z = borderAng(:,3);%border anles in radians
        end
        
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
%FINDING NEAREST NEIGHBOURS
%\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\   
        GlobWidth = [];%Initiates the width array
         Theta = [];%Initiates the Skeleton Angle Array 
        ObWidth = zeros(length(xskel),ObjNum);%Array with widht measures from each column
        point = 0;%counter
        for k = 1:length(xskel) %for all points on the skeleton
             xsk = xskel(k);%x co-ordinate of the skeleton
             ysk = yskel(k);%y co-ordinate of the skeleton
             
             if (xsk>wsize && ysk>wsize && ysk <size(bw,1)-wsize && xsk < size(bw,2)-wsize)%avoiding points close to bounday
                
                [xway,yway] = pathfinder_general(ysk,xsk,skel,wsize);%finds the path of the skeleton around (xsk,ysk) in the neighborhood
                if length(xway)>2 %criteria for line fitting
                [xline,pline,m1] = OrthoLine2(xway,yway,UB);%betst fitted line along the skeleton neighborhood
                fig= figure(20);
%                 fig2 = figure(30);
               
                Theta(k,1) = ysk;%noting the y of skeleton w.r.t which angle is measured
                Theta(k,2) = xsk;%noting the y of skeleton w.r.t which angle is measured
                invtan = -m1;%co-ordinate system adjustments
                if invtan < 0
                ...
                for ob = 1:ObjNum % for each object/column
                     enclosed_boundaries = find(A(:,ob));%identifying the bounday coordinate of each object/column
                   
                    BB = B{ob,1};
                    if ~isempty(enclosed_boundaries)
                        
                        cat = vertcat(BB,B{enclosed_boundaries(:),1});
                        BB = cat;%concatinating interior object boundaries
                    end
%                     BB = B{ob,1};%BOUNDARY COORINATE OF OBJECT 'OB'
                    by = BB(:,1); %y co-ordinate for the border           
                    bx = BB(:,2);%x co-ordinate for the border          
                    
                    [x1,y1,x2,y2,len] = lengthfinder_newmin(xline,pline,xsk,ysk,bx,by,bw,UB);%calculates the length for intersecting bordres
                    if (len ~=0 && y1 ~= 0 && y2 ~=0 && x1 ~= 0 && x2 ~=0)%avoiding zero lengths
                         figure(20), line([x1 x2], [-y1 -y2]);xlim([1 size(bw,2)]);ylim([-size(bw,1) 1]);hold on;
                         plot(xsk,-ysk,'r.');
                        GlobWidth(k,1) = ysk;%y co-ordinate for the skeleton  
                        GlobWidth(k,2) = xsk;%x co-ordinate for the skeleton  
                        GlobWidth(k,3) = y1;%y co-ordinate for the border 1 
                        GlobWidth(k,4) = x1;%x co-ordinate for the border 1  
                        GlobWidth(k,5) = y2;%y co-ordinate for the border 2 
                        GlobWidth(k,6) = x2;%x co-ordinate for the border 2
                        GlobWidth(k,7) = len;%ODC width
                        ObWidth(k,ob) = len;%ODC width
                        point = point + 1;
        ...
        wid = nonzeros(GlobWidth(:,7))*CF;%converting width to microns by multiplying with CF
        wid = wid(wid>wlb & wid <wub);%width exclusion criteria
        %Ang = Theta(any(Theta,2),:);
        %z = Ang(:,3);
        MeanODW = mean(wid);%mean ODC width
        StdODW = std(wid);%standard deviatin of ODC width
        summ = sum(wid);
        len = length(wid);
       
        [counts,centers] = hist(wid,nbins);%histogram of widths
        ...
        [f,x] = ecdf(wid);%cumulative distribution of width
...
%         VarAngle = abs(sqrt(MeanAngleSq-(MeanAngle).^2));
        MeanAngle = atan2(sum(sin(z)),sum(cos(z)));%mean orientation of columns
...
%      VarAngle = 1 - sqrt((sum(sin(z)))^2+(sum(cos(z)))^2)/length(z);
 %     StdAngle = sqrt(-2*log(1-VarAngle))*180/pi;
        ...
        [countsAng,centersAng] = hist(z*180/pi,[9:18:171]);%histogram of border angles with fixed bin boundaries
        ...
        save([path,filesep direc filesep 'Eye' num2str(n2) filesep 'GlobWidth.mat'],'GlobWidth','MeanODW','StdODW','wid','counts','centers','Theta','countsAng','centersAng','MeanAngle','entropy','G','borderAng'); 
        close all;
...
figure, bar(centersAngBoth,countsAngBoth);legend(['ODC Angle Distribution - Mean = ' num2str(H(1,4)) ' Diversity Index' num2str(CI)]);xlabel('ODC Angle (Degrees)'); ylabel('Frequency');
saveas(gcf,[path, filesep direc filesep 'BothEyes' filesep 'AngleDistriHistogramBothEyes.png']); close all;
cd(path);
save([path,filesep direc filesep 'BothEyes' filesep 'CombinedWidth.mat'],'O','W','entropyBoth','H','CI'); 
if xl == 1
